...
router.post("/add", async (req, res) => {
  ...
  if (loaiXuatValue === 'Bán') {
    return res.status(400).json({ message: "Route này không xử lý bán hàng." });
  }
  ...
  try {
    ...
    // 1. Xử lý FEFO và trừ kho
    for (const item of chiTiet) {
      const { MaThuoc, SoLuongXuat } = item; 
      const maThuocTrim = String(MaThuoc).trim();
      const soLuongYeuCau = Number(SoLuongXuat);

      // Lấy giá nhập để làm giá vốn khi xuất bỏ
      const [thuocData] = await conn.query("SELECT GiaNhap FROM Thuoc WHERE MaThuoc = ?", [maThuocTrim]);
      if (thuocData.length === 0) throw new Error(`Thuốc ${maThuocTrim} không tồn tại.`);
      
      // Gán giá vốn vào item để tính tổng tiền
      item.DonGiaXuat = Number(thuocData[0].GiaNhap);
    ...
    await conn.commit();
    res.status(201).json({ message: `Xuất nội bộ thành công! Mã: ${MaPhieuXuat}`, MaPhieuXuat });
